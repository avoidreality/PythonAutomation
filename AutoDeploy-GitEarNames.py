...
diffServers = ['APP1', 'APP2', 'APP3', 'APP4', 'APP5', 'APP6', 'APP7', 'APP8' ]
version = '3.0'
...
class AutoDeploy(object):
    ...
    def connectToNodeManager(self, username, password, host, domain, domain_path):
      ...
      if not r:
         ...
         try:
            if host == "127.0.0.1": # This is changed for public consumption
              #try to connect
              nmConnect(username, password, host, domainName=domain,nmType='plain')
            else:
    ...
    def printState(self, server='FooBarBiz'):
    ...
    def startMS(self, server='FooBarBiz'):
    ...
    def start2(self, server='FooBarBiz'):
    ...
    def stopMS(self, server='FooBarBiz'):
    ...
    def up(self, appName, path, targets):
      ...
      while (not d and count < 4):
        try:
          ...
          print "Trying to deploy again - exception, %s, thrown in upload of new release for: %s" % (str(e),str(appName))
          self.toDisk("Trying to deploy again - exception, %s, thrown in upload of new release for: %s" % (str(e), str(appName)))
          self.activating()
    ...
    def deployT(self):
        ...
        for line in L:
            ...
            if line.startswith('\r\n') or line.startswith('\n'):
                ...
                username = line_s[5].strip()
                password = line_s[6].strip()
                managed_server = line_s[7].strip() #this is column H, This is the target name (I think)
                domain = line_s[3].strip()
                build_path = line_s[11].strip()
                del_ear = line_s[9].strip() #This maybe could be the previous ear name or just the ear name with '.ear' at the end as the field value was previously
                ear_name = line_s[10].strip() #this is the new ear to deploy
                full_path = build_path + os.sep + ear_name 
    

                del_ear = del_ear.replace('.ear', '')
                print "The machine is: %s" % machine
                self.toDisk("The machine is: %s" % machine)
                print "The domain_path is: %s" % domain_path
                self.toDisk("The domain_path is: %s" % domain_path)
                ...
                print "The build_path is: %s" % build_path
                self.toDisk("The build_path is: %s" % build_path)
                print "The ear to delete is: %s" % del_ear
                self.toDisk("The ear to delete is: %s" % del_ear)
                print "The ear_name is: %s" % ear_name
                self.toDisk("The ear_name is: %s" % ear_name)
                print "The full_path is: %s" % full_path
                self.toDisk("The full_path is: %s" % full_path)
                ...
                self.toDisk("*" * 60)
                self.toDisk("The current server is -> '%s'" % machine)
                self.toDisk("*" * 60)
                print "*" * 60
                print "The current server is -> '%s'" % machine
                print "*" * 60
    
                self.stopMS(managed_server)
                self.deleteEar(managed_server,del_ear)
                self.up(ear_name, full_path, managed_server) 

                
                self.toDisk("*" * 60)
                self.toDisk("Moving on to the next server -> %s " % machine) 
                self.toDisk("*" * 60)
   
 		 
                print "*" * 60
                print "Moving on to the next server -> %s" % machine
                print "*" * 60
                #self.loopToContinue()
...
if __name__ == "main" or "__main__":
    ...
    try:
        ...
        for opt, arg in opts:
			...
			#use the -t option for the Git ear names.
